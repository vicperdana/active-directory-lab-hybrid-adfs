...
$allusers = Get-ADUser -Filter * -SearchBase "OU=Users,OU=Employee,DC=vphybridad,DC=lab"
foreach ($user in $allusers){$user | Move-ADObject -TargetPath "OU=Employee,DC=vphybridad,DC=lab"}
...
.\Create-LabUsers.ps1 -AddUpnSuffix -UpnSuffix mevicperdana.onmicrosoft.com -Count 250 -Company "VicPerdana, Inc." -OUPath "OU=Users,OU=Employee,DC=vphybridad,DC=lab"
